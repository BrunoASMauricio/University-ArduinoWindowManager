...
class pongGAME{
  ...
  public:
    pongGAME(int n, unsigned char PongScreen){
      this->nGames = n;
      this->barLeftHeight = SCREEN_HEIGHT/2;
      this->barRightHeight = SCREEN_HEIGHT/2;
      this->winPLAYER1 = 0;
      this->winPLAYER2 = 0;
      this->bal.width =  SCREEN_WIDTH/2 - int(ballSize/2);
      this->bal.height = SCREEN_HEIGHT/2 + int(ballSize/2);
      this->bal.widthspeed = 4;
      this->bal.heightspeed = 0;
      this->ControlledDelay = 100;
      this->screen = PongScreen;
    }
    ...
    int newBallspeed(){
      ...
      if((width < LCDWIDTH-margin-CursorWidth-ballSize and width > margin+CursorWidth)){
        //bouncing on top
        if(height-1 <= margin){
          this->bal.heightspeed = abs(this->bal.heightspeed);
        }
        //bouncing on bottom
        if(height+ballSize+1 >= LCDHEIGHT - margin  ){
          this->bal.heightspeed = -1*abs(this->bal.heightspeed);
        }
        return 0;
      }
      //Right player Contact
      if((height < this->barRightHeight+CursorHeight) and (height+ballSize > this->barRightHeight) and width+ballSize+1 >= LCDWIDTH - margin - CursorWidth){
      ...
      if(width+ballSize > LCDWIDTH - margin - CursorWidth){
        return 1;
      }
    }
    void newRound(){
      ...
      this->barRightHeight = SCREEN_HEIGHT/2;
      this->bal.width =  SCREEN_WIDTH/2 - int(ballSize/2);
      this->bal.height = SCREEN_HEIGHT/2 ;
    ...
    int actualizeGAME(int movplayer1,int movplayer2){
      ...
      NewPosition1 = margin+(movplayer1*(64-2*margin-CursorHeight))/1023;
      //right control
      NewPosition2 = margin+(movplayer2*(64-2*margin-CursorHeight))/1023;
      ...
      Pong_write(PreviousPosition1,PreviousPosition2,0,this->screen);
      Pong_write(NewPosition1,NewPosition2,1,this->screen);
...
void Pong_write(int leftheighty, int rightheighty, int write, unsigned char screen){
  Pong_Paint(leftheighty, margin, CursorHeight, CursorWidth, write, screen);
  Pong_Paint(rightheighty, SCREEN_WIDTH-margin-CursorWidth, CursorHeight, CursorWidth, write, screen);
}
